/*
297. Serialize and Deserialize Binary Tree

Serialization is the process of converting a data structure or object into a sequence of bits so that it can be stored
in a file or memory buffer, or transmitted across a network connection link to be reconstructed later in the same or
another computer environment.
Design an algorithm to serialize and deserialize a binary tree. There is no restriction on how your
serialization/deserialization algorithm should work. You just need to ensure that a binary tree can be serialized to
a string and this string can be deserialized to the original tree structure.
Clarification: The input/output format is the same as how LeetCode serializes a binary tree. You do not necessarily
need to follow this format, so please be creative and come up with different approaches yourself.

Example 1:
Input: root = [1,2,3,null,null,4,5]
Output: [1,2,3,null,null,4,5]

Example 2:
Input: root = []
Output: []

Constraints:
The number of nodes in the tree is in the range [0, 104].
-1000 <= Node.val <= 1000
 */

package binaryTrees.hard;

import binaryTrees.TreeNode;

import java.util.LinkedList;
import java.util.Queue;

public class SerializeDeserializeTree {
    public String serialize(TreeNode root) {
        String serializedString = "";
        if(root==null) return "N,";
        serializedString+=root.val + ",";
        Queue<TreeNode> queue = new LinkedList<>();
        queue.add(root);

        while(!queue.isEmpty()){
            int size = queue.size();
            for(int i=0; i<size; i++){
                TreeNode curr = queue.poll();
                if(curr.left!=null){
                    serializedString+= curr.left.val + ",";
                    queue.add(curr.left);
                }
                else serializedString+="N,";
                if(curr.right!=null){
                    serializedString+= curr.right.val + ",";
                    queue.add(curr.right);
                }
                else serializedString+="N,";
            }
        }
        return serializedString;
    }

    public TreeNode deserialize(String data) {
        if(data=="") return null;

        String[] str = data.split(",");
        if(str[0].equals("N")) return null;

        TreeNode root = new TreeNode(Integer.parseInt(str[0]));
        Queue<TreeNode> queue = new LinkedList<>();
        queue.add(root);

        int index = 1;
        int len = str.length;
        while(!queue.isEmpty() && index<len){
            int size = queue.size();
            for(int i=0; i<size; i++){
                TreeNode curr = queue.poll();
                if(str[index].equals("N")) curr.left = null;
                else{
                    curr.left = new TreeNode(Integer.parseInt(str[index]));
                    queue.add(curr.left);
                }
                index++;

                if(str[index].equals("N")) curr.right = null;
                else{
                    curr.right = new TreeNode(Integer.parseInt(str[index]));
                    queue.add(curr.right);
                }
                index++;
            }
        }
        return root;
    }
}
