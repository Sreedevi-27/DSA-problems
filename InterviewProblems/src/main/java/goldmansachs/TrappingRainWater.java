...
ith line are (i, 0) and (i, height[i]). Find two lines that together with the x-axis form a container,
...
public class TrappingRainWater {
    ...
    public static int maxArea(int[] arr) {
        ...
        while(left<right){
            ...
            while(left < right && arr[left]<=minHeight) left++;
            while(left < right && arr[right]<=minHeight) right--;
        }
        return max;
    }
}
